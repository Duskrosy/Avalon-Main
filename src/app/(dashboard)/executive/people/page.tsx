    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">{totalHeadcount} total</span>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{(pendingLeaves ?? []).length} pending</span>
        </div>
        {(pendingLeaves ?? []).length === 0 ? (
          <p className="px-5 py-8 text-sm text-gray-400 text-center">No pending leave requests.</p>
        ) : (
          <div className="divide-y divide-gray-50">
            {(pendingLeaves ?? []).slice(0, 8).map((leave) => {
              ...
              const colorClass = LEAVE_COLORS[leave.leave_type] ?? "bg-gray-100 text-gray-600";
              const days = Math.max(
                1,
                Math.round(
              ...
              return (
                <div key={leave.id} className="px-5 py-3 flex items-center gap-4">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-gray-400">{days} day{days !== 1 ? "s" : ""}</p>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Leave breakdown · last 7 days + pending</h2>
          {Object.keys(leaveTypeCounts).length === 0 ? (
            <p className="text-sm text-gray-400">No leave data.</p>
          ) : (
            <div className="space-y-3">
              {Object.entries(leaveTypeCounts)
                ...
                .map(([type, count]) => {
                  ...
                  const colorClass = LEAVE_COLORS[type] ?? "bg-gray-100 text-gray-600";
                  const barColor =
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-900">Upcoming approved leaves</h2>
            <p className="text-xs text-gray-400 mt-0.5">Next 30 days</p>
          </div>
          {(upcomingLeaves ?? []).length === 0 ? (
            <p className="px-5 py-8 text-sm text-gray-400 text-center">No upcoming leaves.</p>
          ) : (
            <div className="divide-y divide-gray-50">
